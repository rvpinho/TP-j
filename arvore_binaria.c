...
void busca_arvore_binaria_externa(int elemento, FILE* arq){
    if(elemento!=-1){
        TFunc *f = (TFunc*)malloc(sizeof(TFunc));
        f = pesquisar_arvore_binaria_externa(elemento, arq);
        printf("Funcionário %s de codigo %d encontrado! \n", f->nome, f->cod);
    }
}
...
void excluir_funcionario_arvore_binaria_externa(int codigo, FILE* arq) {
    /*TFunc* funcionario = pesquisar_arvore_binaria_externa(codigo, arq);
    if (funcionario == NULL) {
        printf("Funcionário não encontrado!\n");
        return;
    }*/

    // abre um novo arquivo temporário para escrita binária
    FILE* arv_temp = fopen("arv_temp.dat", "wb+");
    if(arv_temp == NULL) {
        printf("Arquivo arv_temp.dat não encontrado.\n");
        return 1;
    }

    TNoAX aux;
    long posicao = 0;
    while (fread(&aux, sizeof(TNoAX), 1, arq) == 1) {
        printf("Codigo atual %d NÃO FILTRADO! \n", aux.item.cod);
        if (aux.item.cod != codigo) { // se o funcionário não deve ser excluído, copia para o arquivo temporário
            printf("Funcionário %s de codigo %d FILTRADO! \n", aux.item.nome, aux.item.cod);
            fseek(arv_temp, posicao, SEEK_SET);
            fwrite(&aux, sizeof(TNoAX), 1, arv_temp);
            posicao = ftell(arv_temp);
        }
        imprime_chaves_arvore_binaria_externa(arv_temp);
    }
    ...
    fclose(arv_temp);
    fclose(arq);
    remove("ArvoreBinariaExternaFunc.dat"); // exclui o arquivo original
    rename("temp.dat", "ArvoreBinariaExternaFunc.dat"); // renomeia o arquivo temporário para o nome do arquivo original
